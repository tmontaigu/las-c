...
inline double scaling_apply(double scale, double offset, int32_t value)
{
    return (((double)value) * scale) + offset;
}

inline int32_t scaling_unapply(double scale, double offset, double value)
{
    return (int32_t)((value - offset) / scale);
}

inline double las_scaling_apply_x(const las_scaling_t self, int32_t x)
{
    return scaling_apply(self.scales.x, self.offsets.x, x);
}

inline double las_scaling_apply_y(const las_scaling_t self, int32_t y)
{
    return scaling_apply(self.scales.y, self.offsets.y, y);
}

inline double las_scaling_apply_z(const las_scaling_t self, int32_t z)
{
    return scaling_apply(self.scales.z, self.offsets.z, z);
}

inline int32_t las_scaling_unapply_x(const las_scaling_t self, double x)
{
    return scaling_unapply(self.scales.x, self.offsets.x, x);
}

inline int32_t las_scaling_unapply_y(const las_scaling_t self, double y)
{
    return scaling_unapply(self.scales.y, self.offsets.y, y);
}

inline int32_t las_scaling_unapply_z(const las_scaling_t self, double z)
{
...
struct las_header_t
{
    ...
    /// and the values are stored as uint32_t
